...
app.use(bodyParser.json({
  verify: verifyRequestSignature // Read function for more info
}));
...
app.post('/webhook/', function (req, res) {
  ...
  if (data.object == 'page') {
    ...
    data.entry.forEach(function (pageEntry) {
      ...
      // Iterate over each messaging event - reading message by user
...
function receivedMessage(event) {
  ...
  if (messageText) { //send message to api.ai
 ...
 *
 */
function receivedPostback(event) { // When user clicks on a generic item in thread
  var senderID = event.sender.id;
  var recipientID = event.recipient.id;
...
function verifyRequestSignature(req, res, buf) {
  ...
  if (!signature) {
    ...
    var expectedHash = crypto.createHmac('sha1', config.FB_APP_SECRET) // Verification
      .update(buf)
      .digest('hex');
